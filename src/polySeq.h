...
public:
    ...
    void setup() {
        ...
        lastSteps.resize(MAX_SLIDERS, -1);  // Initialize with -1 to ensure first step is always calculated
    ...
    void updateOutputs() {
        ...
        for (int i = 0; i < numTracks; i++) {
            ...
            if (currentSize > 0) {
                ...
                if (probabilistic) {
                    if (step != lastSteps[i] || phasor < lastPhasors[i]) {  // Check if we've entered a new step or if phasor has reset
                        float probability = vectorValues[i][step];
                        float randomValue = ofRandom(1.0f);
                        currentOutputs[i] = (randomValue < probability) ? 1.0f : 0.0f;
                        lastSteps[i] = step;
                    }
                    lastPhasors[i] = phasor;  // Update last phasor value
                } else {
